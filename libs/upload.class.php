    ...
    function process($server_path = null) {
        ...
        if ($this->processed) {
            ...
            if ($this->file_safe_name) { // formats the name
                $this->file_dst_name_body = strtr($this->file_dst_name_body, ' ?Á?üÌÛÌ?ÌâÌÄÌãÌÉÌàÌöÌäÌ?ÌÜÌÎÌ?Ì?Ì?ÌÔÌÕÌÒÌÓÌ¥ÌÐÌ÷ÌªÌ?ÌÝÌÏÌ?Ì ÌÁÌ¢Ì£Ì?Ì´Ì¤Ì¬Ì©Ì»ÌÇÌÂÌ?Ì¨ÌøÌ±Ì?Ì?Ì«ÌµÌ¦ÌüÌ?Ì¼ÌÈÌ?Ì?ÌÀ', 'SZszYAAAAAACEEEEIIIINOOOOOOUUUUYaaaaaaceeeeiiiinoooooouuuuyy');
                $this->file_dst_name_body = strtr($this->file_dst_name_body, array('Ì?' => 'TH', 'Ì?' => 'th', 'Ì?' => 'DH', 'Ì¡' => 'dh', 'ÌÙ' => 'ss', 'Õ' => 'OE', 'Ò' => 'oe', 'Ì ' => 'AE', 'Ì?' => 'ae', 'åµ' => 'u'));
            ...
            if ($image_manipulation) {
                ...
                if ($this->processed && $image_src) {
                    ...
                    if ($gd_version >= 2 && $this->image_unsharp && is_numeric($this->image_unsharp_amount) && is_numeric($this->image_unsharp_radius) && is_numeric($this->image_unsharp_threshold)) {
                        ...
                        // Unsharp mask algorithm by Torstein HÌünsi 2003-07. 
